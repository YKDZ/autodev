...
export class GithubWebhookHandler {
  ...
  async handle(event: WebhookEvent): Promise<void> {
    ...
    try {
      ...
      updateWebhookDeliveryStatus(this.ctx.workspaceRoot, deliveryId, result);
    } catch (err) {
  ...
  private async handleIssueCommentEvent(
    ...
    const resourceVersion =
      comment.updated_at ?? comment.created_at ?? String(comment.id);
  ...
  private extractRepo(payload: unknown): string | null {
    if (
      ...
      if (repo !== null && typeof repo === "object" && "full_name" in repo) {
